...
template <size_t populationSize, size_t numGenerations, double mutationRate>
static void BM_serial_Optimized(benchmark::State& state) {
    // Run the benchmark for each state iteration
    for (auto _ : state) {
        // Perform genetic optimization with the specified parameters and fitness function
        auto bestChromosome = geneticOptimizationSerialOptimized<populationSize, numGenerations, mutationRate>(rastriginFunction);
...
template <size_t populationSize, size_t numGenerations, double mutationRate>
static void BM_TBB_Optimized(benchmark::State& state) {
    // Run the benchmark for each state iteration
    for (auto _ : state) {
        // Perform genetic optimization with the specified parameters and fitness function
        auto bestChromosome = geneticOptimizationTBBOptimized<populationSize, numGenerations, mutationRate>(rastriginFunction);
...
BENCHMARK(BM_serial_Optimized<10000, 10000, 0.4>)->Unit(benchmark::kMillisecond);
BENCHMARK(BM_TBB_Optimized<10000, 10000, 0.4>)->Unit(benchmark::kMillisecond);
